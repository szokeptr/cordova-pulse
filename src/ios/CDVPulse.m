...
@interface CDVPulse ()<AVCaptureVideoDataOutputSampleBufferDelegate>
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    ...
    if(s>0.5 && v>0.5) {
       ...
       // NSLog(@"have we collected enough frames for the filter to settle?");
...
-(void) update {
    ...
    if(avePeriod==INVALID_PULSE_PERIOD) {
        // no value available
        //NSLog(@"No value available.... :( %f", avePeriod);
        //self.pulseRate.text=@"--";
    } else {
        ...
        CDVPluginResult* result = [CDVPluginResult
                                   ...
                                   messageAsString: [NSString stringWithFormat:@"%0.0f", pulse]];
